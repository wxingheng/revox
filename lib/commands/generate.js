...
module.exports = function () {
  ...
  try {
    let file_name_path = path.join(templateDir, `./tmp/${file_name}`);
    console.log(file_name_path);
    // 判断 file_name_path 是否存在
    if (!fs.existsSync(file_name_path)) {
      // console.error("文件或文件夹不存在:", file_name_path);
      // 尝试在 file_name_path 后面加上 .revox.json
      file_name_path = `${file_name_path}.revox.json`;
      if (!fs.existsSync(file_name_path)) {
        console.error("文件或文件夹不存在:", file_name_path);
        return;
      }
    ...
    console.log("正在处理文件:", file_name_path);
